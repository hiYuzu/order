    ...
    base: "js/"
        ...
        function initPartsTable(tableData) {
            table.render({
                ...
                , cols: [[
                    ...
                    , {field: 'partId', title: '部件ID', minWidth: '100', align: 'center'}
                    , {field: 'partTypeCode', title: '部件类型编码', minWidth: '100', align: 'center'}
                    , {field: 'partTypeName', title: '部件类型', minWidth: '180', align: 'center'}
                    , {field: 'partNo', title: '部件序号', minWidth: '230', align: 'center', edit: 'text'}
                    , {field: 'partMemo', title: '部件备注', minWidth: '245', align: 'center', edit: 'text'}
                ]]
                , limit: 100
                , done: function (res, curr, count) {
                    $("[data-field='partId']").css('display', 'none');
        ...
        function initFormData() {
            if (row != null && row != '') {
                ...
                if(row.testId != null && !isNaN(row.testId)){
                ...
                if (row.beginTime != null) {
                    ...
                    $("#beginTime").val(row.completeDate);
                }
                $("#endTime").val(row.endTime);
                $("#testMemo").val(row.testMemo);
                //部件列表
                $.ajax({
        ...
        form.on("submit(updateTest)", function (data) {
            ...
            $.ajax({
                url: "../../AssembleTestController/operateAssembleTest",
                type: "post",
                dataType: "json",
                data: dataPara,
                error: function () {
                    top.layer.close(index);
                    top.layer.msg("组装测试操作失败！");
                },
                success: function (json) {
                    top.layer.close(index);
                    if (json.result) {
                        top.layer.msg("组装测试操作成功！");
                        layer.closeAll("iframe");
                        //刷新父页面
                        parent.location.reload();
                    } else {
                        top.layer.msg("组装测试操作失败,失败信息为:" + json.detail);
                    }
                }
            });
            return false;
