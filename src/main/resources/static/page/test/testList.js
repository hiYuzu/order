...
layui.config({
    ...
    function startSearchTest() {
        var pn = $("#searchPn").val();
        var finishFlag = $("#searchFinishFlag").val();
        var snCode = $("#searchSnCode").val();
        var jobNo = $("#searchJobNo").val();
        searchTest(pn, finishFlag, snCode, jobNo);
    }

    function searchTest(pn, finishFlag, snCode, jobNo) {
        table.render({
            id: 'testId,assembleId,pnCode,assembleStatusCode',
            elem: '#testListTable',
            url: '../../AssembleTestController/getAssembleTest',
            method: 'post',
            where: {
                pnCode: pn,
            ...
            cols: [[{field: 'pnName', title: 'P/N', minWidth: '100', sort: true}
                ...
                , {field: 'beginTime', title: '测试开始时间', minWidth: '120', sort: true}
                , {field: 'endTime', title: '测试结束时间', minWidth: '120', sort: true}
                , {field: 'testMemo', title: '测试备注', minWidth: '160'}
                , {field: 'optUserName', title: '操作者', minWidth: '120'}
                , {field: 'optTime', title: '操作时间', minWidth: '180'}
                , {title: '操作', fixed: 'right', minWidth: 160, align: 'center', toolbar: '#barTestList'}
            ]]
        });
    }

    //监听工具条
    table.on('tool(testListFilter)', function (obj) {
        var data = obj.data;
        if (data == null || data == "") {
