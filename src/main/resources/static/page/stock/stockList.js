...
layui.config({
    ...
    function startSearchStock() {
        var pn = $("#searchPn").val();
        var finishFlag = $("#searchFinishFlag").val();
        var snCode = $("#searchSnCode").val();
        var jobNo = $("#searchJobNo").val();
        searchStock(pn, finishFlag, snCode, jobNo);
    }

    function searchStock(pn, finishFlag, snCode, jobNo) {
        table.render({
            ...
            cols: [[{field: 'pnName', title: 'P/N', minWidth: '100', sort: true}
                ...
                , {title: '操作', fixed: 'right', minWidth: 160, align: 'center', toolbar: '#barStockList'}
            ]]
        });
    }

    //监听工具条
    table.on('tool(stockListFilter)', function (obj) {
        var data = obj.data;
        if (data == null || data == "") {
