...
layui.config({
    ...
    function searchDeliver(ctCode, ctType, customerName, deliverStatus, finishFlag) {
        table.render({
            id: 'deliverId,contractTypeCode,deliverStatusCode,goodsModelList',
            ...
            page: true //开启分页
            ,
            cols: [[{field: 'customerName', title: '客户名称', minWidth: '160', sort: true}
    ...
    //监听工具条
    table.on('tool(outsourcingListFilter)', function (obj) {
        var data = obj.data;
        if (data == null || data == "") {
        ...
        if (obj.event === 'select') {
            ...
            layer.confirm('确认删除客户名称是\'' + data.customerName + '\'的外购单吗', function (index) {
                var idArray = [];
                ...
                $.ajax({
                    ...
                    error: function (json) {
                        ...
                        top.layer.msg("外购单删除失败！");
                    },
                    success: function (json) {
                        top.layer.close(index);
                        if (json.result) {
                            top.layer.msg("外购单删除成功！");
                            var ctCode = $("#searchCtCode").val();
                            var ctType = $("#searchCtType").val();
                            var customerName = $("#searchCustomerName").val();
                            var deliverStatus = $("#searchDeliverStatus").val();
